...
function closeAllModals() {
  const modals = [
    ...
    document.getElementById('remote-logs-modal')
  ];
  let closedAny = false;
  modals.forEach(m => {
...
// ==========================================
// LIVE REMOTE LOGS VIEWER (FIREBASE TELEMETRY)
// ==========================================

let cachedRemoteLogs = [];
let activeRemoteFilter = 'all';

async function openRemoteLogsModal() {
  ...
  const deviceLabelEl = document.getElementById('current-device-id-label');
  if (deviceLabelEl && window.Telemetry) {
    const dev = window.Telemetry.getDeviceInfo();
    deviceLabelEl.textContent = `${dev.label} • ID: ${window.Telemetry.getDeviceId()}`;
  }

  await loadAndRenderRemoteLogs();
}
...
async function loadAndRenderRemoteLogs() {
  const listEl = document.getElementById('remote-logs-list');
  const countBadge = document.getElementById('remote-logs-count-badge');
  if (!listEl) return;

  listEl.innerHTML = '<p class="text-neutral-400 text-center py-8 animate-pulse font-mono">Consultando Firebase Firestore en tiempo real...</p>';

  if (!window.Telemetry) {
    listEl.innerHTML = '<p class="text-red-400 text-center py-8">Módulo Telemetry no inicializado.</p>';
    return;
  }

  cachedRemoteLogs = await window.Telemetry.fetchLogs(80);
  if (countBadge) countBadge.textContent = `${cachedRemoteLogs.length} logs`;

  renderFilteredRemoteLogs();
}

function renderFilteredRemoteLogs() {
  const listEl = document.getElementById('remote-logs-list');
  if (!listEl) return;

  let filtered = cachedRemoteLogs;
  if (activeRemoteFilter === 'error') {
    filtered = cachedRemoteLogs.filter(l => l.level === 'error');
  } else if (activeRemoteFilter === 'iphone') {
    filtered = cachedRemoteLogs.filter(l => (l.os || '').toLowerCase().includes('iphone') || (l.deviceLabel || '').toLowerCase().includes('iphone'));
  }

  if (filtered.length === 0) {
    listEl.innerHTML = '<p class="text-neutral-500 text-center py-8 font-mono">No hay logs registrados para este filtro.</p>';
    return;
  }
  ...
  filtered.forEach(log => {
    ...
    item.className = 'py-2.5 flex flex-col gap-1 border-b border-neutral-800 last:border-0 hover:bg-neutral-800/40 px-2 transition-colors';

    let levelBadge = '<span class="px-1.5 py-0.2 text-[9px] font-bold uppercase bg-blue-950 text-blue-400 border border-blue-800">INFO</span>';
    if (log.level === 'error') {
      levelBadge = '<span class="px-1.5 py-0.2 text-[9px] font-bold uppercase bg-red-950 text-red-400 border border-red-800">ERROR</span>';
    } else if (log.level === 'success') {
      levelBadge = '<span class="px-1.5 py-0.2 text-[9px] font-bold uppercase bg-emerald-950 text-emerald-400 border border-emerald-800">ÉXITO</span>';
    } else if (log.level === 'warn') {
      levelBadge = '<span class="px-1.5 py-0.2 text-[9px] font-bold uppercase bg-amber-950 text-amber-400 border border-amber-800">AVISO</span>';
    }

    const timeStr = log.timestamp ? new Date(log.timestamp).toLocaleTimeString() : '—';
    const dateStr = log.timestamp ? new Date(log.timestamp).toLocaleDateString() : '';

    const deviceIcon = (log.os || '').toLowerCase().includes('iphone') || (log.deviceLabel || '').toLowerCase().includes('iphone')
      ? '📱' : ((log.mode || '').includes('Extension') ? '💻' : '🌐');

    item.innerHTML = `
      <div class="flex flex-wrap items-center justify-between gap-1.5 text-[10px]">
        <div class="flex items-center gap-2">
          ${levelBadge}
          <span class="text-neutral-300 font-bold">${deviceIcon} ${log.deviceLabel}</span>
          <span class="text-neutral-500">|</span>
          <span class="text-prada-ochre font-semibold">👤 ${log.username}</span>
        </div>
        <span class="text-neutral-500 text-[10px]">${timeStr} <span class="text-neutral-600">(${dateStr})</span></span>
      </div>
      <div class="text-xs text-neutral-100 font-medium pl-1 break-words">
        ${escapeHtml(log.message)}
      </div>
      ${log.details && log.details !== '{}' && log.details !== '""' ? `
        <details class="text-[10px] text-neutral-400 pl-1 mt-0.5 cursor-pointer">
          <summary class="hover:text-neutral-200">Ver detalles / Stack trace</summary>
          <pre class="bg-black/60 p-2 mt-1 border border-neutral-800 text-[10px] text-neutral-300 overflow-x-auto whitespace-pre-wrap">${escapeHtml(log.details)}</pre>
        </details>
      ` : ''}
    `;
...
// Event Listeners for Live Remote Logs
document.addEventListener('DOMContentLoaded', () => {
  const btnOpenRemoteLogs = document.getElementById('btn-open-remote-logs');
  const tickerTelemetryBtn = document.getElementById('ticker-telemetry-btn');
  const btnCloseRemoteLogs = document.getElementById('btn-close-remote-logs');
  const btnRefreshRemoteLogs = document.getElementById('btn-refresh-remote-logs');

  if (btnOpenRemoteLogs) btnOpenRemoteLogs.addEventListener('click', openRemoteLogsModal);
  if (tickerTelemetryBtn) tickerTelemetryBtn.addEventListener('click', openRemoteLogsModal);
  if (btnCloseRemoteLogs) btnCloseRemoteLogs.addEventListener('click', closeRemoteLogsModal);
  if (btnRefreshRemoteLogs) btnRefreshRemoteLogs.addEventListener('click', loadAndRenderRemoteLogs);

  // Filter chips listeners
  document.querySelectorAll('.remote-filter-btn').forEach(btn => {
    btn.addEventListener('click', (e) => {
      document.querySelectorAll('.remote-filter-btn').forEach(b => {
        b.className = 'px-2.5 py-1 bg-pure-white border border-hairline-dark text-pitch-black font-bold uppercase cursor-pointer remote-filter-btn';
      });
      btn.className = 'px-2.5 py-1 bg-pitch-black text-pure-white font-bold uppercase cursor-pointer remote-filter-btn active';
      activeRemoteFilter = btn.dataset.filter || 'all';
      renderFilteredRemoteLogs();
    });
  });
});
